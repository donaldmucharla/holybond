...
// fixed timestamp (no Date.now -> no hydration mismatch)
...
export default function SearchClient() {
  ...
  // mount guard (localStorage reads only after mount)
  ...
  // must apply at least one filter
        ...
        <div className="lg:col-span-8 space-y-4">
          <div className="flex items-center justify-between rounded-3xl border border-white/10 bg-white/5 p-4">
            <div className="text-sm text-slate-300">
              Results: <span className="text-white font-semibold">{mounted ? results.length : 0}</span>
            </div>
            <div className="text-xs text-slate-500">
              {!mounted ? "Loading..." : hasAnyFilter ? "Tip: click “View” for full profile" : "Add filters and click Apply"}
            </div>
          </div>
          ...
          {!mounted ? (
            <div className="rounded-3xl border border-white/10 bg-white/5 p-10 text-center">
              ...
              <div className="mt-2 text-sm text-slate-400">Preparing search page.</div>
            </div>
          ) : !hasAnyFilter ? (
            <div className="rounded-3xl border border-white/10 bg-white/5 p-10 text-center">
